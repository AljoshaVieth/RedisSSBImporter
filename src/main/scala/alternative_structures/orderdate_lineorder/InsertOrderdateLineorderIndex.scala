...
object InsertOrderdateLineorderIndex extends Inserter {
		...
		data.foreach(line => {
			...
			dataStructure.foreach(key => {
				if (valueIterator.hasNext) {
					pipeline.hset((databaseName + ":orderdate:" + values(5) + ":" + values(0) + ":" + values(1)).getBytes, key.getBytes, valueIterator.next().getBytes())
				}
			})
		})
		pipeline.sync()
